<?php   
    ...
    if (empty($usuario)) {
        header("Location: formusuario.php?error=El Usuario Es Requerido");
        exit();
    }elseif (empty($contrasena)) {
        header("Location: formusuario.php?error=La contraseña Es Requerida");
        exit();
    }else{
        ...
        if (mysqli_num_rows($result) === 1) {
            ...
            if ($row['usuario'] === $usuario && $row['contrasena'] === $contrasena) {
                ...
                header("Location:/vista/formusuario.php?error=El usuario o la clave son incorrectas");
                exit();
            }
